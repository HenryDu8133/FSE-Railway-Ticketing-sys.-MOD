...
import net.fsefmgftc.fseticket.client.audio.SharedStreamingGroup;
import net.fsefmgftc.fseticket.client.audio.StreamingAudioSource;
...
public class FSEAudioStream implements AudioStream {
    ...
    public boolean isStreamReady() {
        ...
        if (sfmt != null
            ...
            streamReady = true;
            FseticketMod.LOGGER.info("AudioStream: prebuffer ready, format=" + sfmt);
        }
        return streamReady;
    }
    ...
    public void push(SpeakerAudioPacket packet) {
        if (closed) return;
    ...
    private void startStreaming(SpeakerAudioPacket packet) {
        if (packet.streamUrl == null || packet.streamUrl.isEmpty()) {
            FseticketMod.LOGGER.warn("AudioStream: no URL for streaming packet");
            return;
        }
        isStreaming = true;
        streamReady = false;
        ...
        if (packet.syncGroupId != null && packet.syncGroupSize > 1) {
            ...
            streamingSource = null;
            FseticketMod.LOGGER.info("AudioStream: joined shared " + packet.format + " group " + packet.syncGroupId + " from " + packet.streamUrl);
        } else {
            ...
            streamingSource.setMetadataListener((rawTitle, station, genre, desc) -> {
                try {
                    net.neoforged.neoforge.network.PacketDistributor.sendToServer(
                        new net.fsefmgftc.fseticket.network.IcyMetaPacket(
                            sourceId, rawTitle, station, genre, desc));
                } catch (Exception e) {
                    FseticketMod.LOGGER.warn("AudioStream: failed to send ICY meta -?" + e.getMessage());
                }
            });

            streamingSource.start();
            FseticketMod.LOGGER.info("AudioStream: started " + packet.format + " from " + packet.streamUrl);
        }
    }
    ...
    private void pushPCM(byte[] raw, javax.sound.sampled.AudioFormat fmt) {
        ...
        synchronized (queue) {
            if (queue.size() >= MAX_QUEUE_CHUNKS) {
                FseticketMod.LOGGER.warn("AudioStream: dropping PCM chunk; queue full");
                return;
            }
        ...
        try {
            ...
            if ("ogg".equals(kind)) {
                ...
                if (src.length > MAX_DECODED_PCM_BYTES) {
                    FseticketMod.LOGGER.warn("AudioStream: decoded " + label + " too large, dropping");
                    return;
                }
                ...
                if (srcCh == 1) {
                    ...
                    if (srcCh <= 0 || srcCh > 8) { FseticketMod.LOGGER.warn("AudioStream: invalid channel count " + srcCh); return; }
                    int frames = src.length / (srcCh * 2);
                    if ((long)frames * 2L > MAX_DECODED_PCM_BYTES) { FseticketMod.LOGGER.warn("AudioStream: mono decode too large"); return; }
            ...
            FseticketMod.LOGGER.warn("AudioStream: unsupported " + label + " -?" + e.getMessage());
        } catch (Exception e) {
            FseticketMod.LOGGER.error("AudioStream: " + label + " decode failed -?" + e);
        }
    }
}
