...
public class StreamingAudioSource {
    ...
    public void start() {
        if (running.compareAndSet(false, true)) {
            ...
            streamThread.start();
            FseticketMod.LOGGER.info("StreamingAudio: Started " + type + " from " + url);
        }
    }
    ...
    public void stop() {
        ...
        pcmQueue.clear();
        FseticketMod.LOGGER.info("StreamingAudio: Stopped");
    }
    ...
    private void streamLoop() {
        try {
            ...
            if (!stopped.get()) {
                FseticketMod.LOGGER.error("StreamingAudio: Stream error -?" + e.getMessage());
            }
        } finally {
            running.set(false);
        }
    ...
    private void streamMP3() throws IOException {
        ...
        FseticketMod.LOGGER.info("StreamingAudio: icy-metaint=" + metaInterval);
        ...
        try {
            while (!stopped.get()) {
                ...
                try {
                    ...
                    FseticketMod.LOGGER.warn("StreamingAudio: Bitstream sync error, resyncing -?" + e.getMessage());
                    continue;
                }

                if (header == null) {
                    
                    FseticketMod.LOGGER.info("StreamingAudio: MP3 stream ended (server EOF)");
                    break;
                }
                ...
                if (!formatDetected) {
                    ...
                    formatDetected = true;
                    FseticketMod.LOGGER.info(String.format(
                ...
                try {
                    ...
                    FseticketMod.LOGGER.warn("StreamingAudio: Frame decode error -?" + e.getMessage());
        ...
        while (!stopped.get()) {
            ...
            if (currentPlaylist == null ||
                ...
                if (currentPlaylist.type == HLSPlaylistParser.PlaylistType.MASTER) {
                    ...
                    if (variant == null) {
                        FseticketMod.LOGGER.error("StreamingAudio: No suitable HLS variant");
                        return;
                    }
                    FseticketMod.LOGGER.info("StreamingAudio: HLS variant " + variant.bandwidth + " bps");
    ...
    private void streamTS() throws IOException {
        ...
        try (InputStream is = conn.getInputStream()) {
            ...
            FseticketMod.LOGGER.info("StreamingAudio: Demuxed " + frames.size() + " TS frames");
    ...
    private void playTSSegment(String segUrl) {
        try {
            ...
            FseticketMod.LOGGER.warn("StreamingAudio: TS segment failed -?" + e.getMessage());
        }
    }
    ...
    private void playAACSegment(String segUrl) {
        try {
            ...
            FseticketMod.LOGGER.warn("StreamingAudio: AAC segment failed -?" + e.getMessage());
        }
    }
    ...
    private byte[] decodeAudioFrame(TSDemuxer.AudioFrame frame) {
        try {
            ...
            FseticketMod.LOGGER.warn("StreamingAudio: frame decode failed -?" + e.getMessage());
            return null;
        }
    }
        ...
        while (accumLen - start >= CHUNK_BYTES || (force && start < accumLen)) {
            ...
            try {
                if (!pcmQueue.offer(chunk, 50, TimeUnit.MILLISECONDS)) {
                    FseticketMod.LOGGER.warn("StreamingAudio: PCM queue full, dropping chunk");
                }
    ...
    private void notifyMetadata(String rawTitle, String station, String genre, String desc) {
        ...
        catch (Exception e) { FseticketMod.LOGGER.warn("StreamingAudio: metadata listener: " + e); }
    }
    ...
    private final class IcyInputStream extends FilterInputStream {
        ...
        private void checkMetaBoundary() throws IOException {
            ...
            if (metaBytes > 4096) {
                FseticketMod.LOGGER.warn("StreamingAudio: ICY meta block too large (" + metaBytes + "), skipping");
            ...
            FseticketMod.LOGGER.info("StreamingAudio: ICY metadata = " + metaStr);
