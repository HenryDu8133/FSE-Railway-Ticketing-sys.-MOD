    ...
    private static final class Session {
            ...
            this.source.setMetadataListener((rawTitle, station, genre, desc) -> {
                try {
                    net.neoforged.neoforge.network.PacketDistributor.sendToServer(
                        new net.fsefmgftc.fseticket.network.IcyMetaPacket(metadataSourceId, rawTitle, station, genre, desc));
                } catch (Exception e) {
                    FseticketMod.LOGGER.warn("SharedStreamingGroup: failed to send ICY meta -?" + e.getMessage());
                }
            });
        }
        ...
        private void startDecodeIfReady() {
            ...
            if (running.compareAndSet(false, true)) {
                ...
                FseticketMod.LOGGER.info("SharedStreamingGroup: started shared session " + groupId + " for " + type + " " + url + " with " + taps.size() + "/" + expectedTaps + " taps");
            }
        }
        ...
        private void distributeLoop() {
            try {
                ...
                if (running.get()) FseticketMod.LOGGER.warn("SharedStreamingGroup: distributor error -?" + e.getMessage());
            } finally {
                forceClose();
            }
        }
        ...
        private void forceClose() {
            ...
            if (running.compareAndSet(true, false)) {
                ...
                taps.clear();
                remove(groupId, this);
                FseticketMod.LOGGER.info("SharedStreamingGroup: closed session " + groupId);
            }
        }
    }
}
