...
public class ICGUIScreen extends AbstractContainerScreen<ICGUIMenu> implements FseticketModMenus.ScreenAccessor {
    ...
    @Override
    public void updateMenuState(int t, String n, Object s) {
    }
    ...
    @Override
    public boolean keyPressed(int k, int b, int c) {
        if (k == 256) {
    ...
    private String nv(String s) {
        return s == null || s.isEmpty() || s.equals("\"\"") ? "---" : s;
    }
    ...
    protected void renderLabels(GuiGraphics g, int mx, int my) {
        ...
        g.drawString(font, Component.literal(String.format("%.2f", m.balance)).withStyle(FONT), 14, -15, -16758869, false);
        g.pose().popPose();
    ...
    public void init() {
        super.init();
        String cid = nv(menu.cardId);
        if (!cid.equals("---"))
            qrTexture = QRCodeGenerator.getOrGenerate("https://ticket.fse-media.group/ic/" + cid, 64);
    }
}
