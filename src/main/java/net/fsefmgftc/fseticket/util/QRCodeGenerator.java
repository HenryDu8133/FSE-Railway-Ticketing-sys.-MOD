    ...
    public static ResourceLocation getOrGenerate(String url, int size) {
        ...
        for (int x = 0; x < size; x++)
            for (int y = 0; y < size; y++)
                blank.setPixelRGBA(x, y, 0x00000000);
        DynamicTexture texture = new DynamicTexture(blank);
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                BufferedImage bufferedImage = ImageIO.read(new URL(api));
                NativeImage nativeImage = new NativeImage(size, size, false);
                for (int x = 0; x < size; x++) {
                    for (int y = 0; y < size; y++) {
                        int rgb = bufferedImage.getRGB(x, y);
                        int r = (rgb >> 16) & 0xFF, g = (rgb >> 8) & 0xFF, b = rgb & 0xFF;
                        nativeImage.setPixelRGBA(x, y, (r > 200 && g > 200 && b > 200) ? 0x00000000 : 0xFF000000);
                    }
                }
